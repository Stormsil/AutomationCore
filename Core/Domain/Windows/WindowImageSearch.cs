...
namespace AutomationCore.Core.Services
{
    ...
    {
        private readonly ITemplateStorage _store;
        ...
        public WindowImageSearch(ITemplateStorage store = null, object settings = null)
        {
            _store = store ?? new FileTemplateStorage("templates");

            var adapter = new TemplateStorageAdapter(_store);
            _matcher = new TemplateMatcherService(adapter);
            // _captureSettings = settings; // TODO: Implement capture settings
        }
        ...
        private static MatchOptions MapToMatchOptions(TemplateMatchOptions t)
        {
            var o = new MatchOptions
            {
                Threshold = t.Threshold,
                UseMultiScale = Math.Abs(t.ScaleMax - t.ScaleMin) > 1e-9,
                ScaleRange = new ScaleRange(t.ScaleMin, t.ScaleMax, t.ScaleStep),
